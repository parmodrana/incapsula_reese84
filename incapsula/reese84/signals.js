    ...
    signalPath.traverse({
		...
		*/
        ...
        (() => {
          ...
          //const codeNext = generate(path.getNextSibling().node).code;
		   ...
		   /**
		   * Anh's notes: codeNext looks like 
		   * 
		   * var Qm = window["Object"]["getOwnPropertyNames"](window);
			  ...
			  so, it's not path.getNextSibling() anymore
		   */
        ...
        (() => {
          ...
          if(code.endsWith(`["openDatabase"] ? true : false;`)){
			//Anh's notes: The code with openDatabase looks like: 
			/**
			 * try {
                if (pf() !== undefined) {
			 ...
			 */
			//So changing "path.getSibling(path.key - 1)" to "path.getSibling(path.key + 1)" 
      ...
      ExpressionStatement(path){
        ...
        (() => {
		  ...
		  /**
		   * Anh's Notes: get(`block.body.1.consequent.body.5.expression.left.property`); 
		   * 
		   * to 
		   * 
		   * get(`block.body.1.consequent.body.4.expression.left.property`); 
		   * 
		   * Code looks like: 
		   * 
		   * try {
          ...
          const leftProp = path.getSibling(path.key+1).get(`block.body.1.consequent.body.4.expression.left.property`);
        ...
        (() => {
		  ...
		  /**
		   * Anh's notes: const leftProp = path.getSibling(path.key + 2).get(`expression.left.property`);
		   */

          const leftProp = path.getSibling(path.key + 6).get(`expression.left.property`);

		  console.log(leftProp.node.type, getPropertyValue(leftProp));
