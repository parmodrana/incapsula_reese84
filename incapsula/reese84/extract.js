...
function createEncoderFromPath({
  ...
  const handleWhileXorBySeed = (path) => {
    ...
    const startSlice = path.get(`body.body.1.declarations.0.init.left.object.arguments.0.value`).node;
    const endSlice = path.get(`body.body.1.declarations.0.init.left.object.arguments.1.value`).node;
  ...
  if (path.type === `WhileStatement`) {
    ...
    switch (body.length) {
      case 2:
        ...
        /**
         * There are 2 types of body: 
         *    Type 1: // ir.push((rX[y9] >> wg | rX[y9] << 8 - wg) & 255);
         *    Type 2: // Zj.push(g5() & 255);
...
function getSignalsPaths(ast) {
  ...
  traverse(ast, {
    ...
    Program(path) {
      ...
      const mainFuncPath = path.get(`body.0.expression.callee.body.body`).slice(-2)[0]; //slice(-2) get 2 last items      
      const bodyPaths = mainFuncPath.get(`body.body.0.expression.right.body.body.0.block.body.2.expression.arguments.1.body.body.0.block.body`);
